...
const actionMiddleWare: Middleware = ({
  ...
  if (action.type.includes("ADD_USER")) {
  ...
  } else if (action.type.includes("UPDATE_USER")) {
    const [
      START_UPDATE_USER,
      SUCCESS_UPDATE_USER,
      FAIL_UPDATE_USER
    ...
    next({ ...action, type: START_UPDATE_USER });

    // Here we will dispatch SUCCESS OR FAIL based on some condition or try/catch
    try {
      const response: AxiosResponse = await action.promise;
      if (response.status === 200) {
        return next({ ...action, type: SUCCESS_UPDATE_USER });
      }
    } catch (error) {
      return next({ ...action, type: FAIL_UPDATE_USER });
    }
  }
